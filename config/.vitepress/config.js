...
function generateSidebar(lang) {
    ...
    return [
      ...
      },
      {
        ...
        items: [
          ...
          { text: '2. Le wagon invisible et la particule を', link: '/fr/2-the-invisible-carriage-and-the-を-particle' },
  ...
  // Configuration pour les autres langues (anglais et russe)
  const lessons = []
  for (let i = 1; i <= 97; i++) {
    lessons.push({
      text: `${lang === 'en' ? 'Lesson' : 'Урок'} ${i}`,
      link: `/${lang}/lesson-${i}`
    })
  }
  ...
  return [
    {
      text: lang === 'en' ? 'Introduction' : 'Введение',
      items: [
        { text: lang === 'en' ? 'Home' : 'Главная', link: `/${lang}/` },
        { text: lang === 'en' ? 'About' : 'О проекте', link: `/${lang}/about` }
      ]
    },
    {
      text: lang === 'en' ? 'Lessons' : 'Уроки',
      items: lessons.slice(0, 20)
    }
  ]
}
