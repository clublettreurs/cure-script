...
**С ичидан-глаголами мы можем присоединить его к универсальной ичидан-основе** (как вы знаете, с ичидан-глаголами мы всегда, что бы мы с ними ни делали, просто отбрасываем <code>-る</code> и добавляем то, что хотим). Так, например, <code>見る</code> может стать <code>見まい / みまい</code>, **но так же часто это** <code>見るまい</code>.
...
Строго говоря, особенно в прошлом, использование ичидан-глаголов целиком с <code>まい</code> на конце, вероятно, было неграмматичным, но сейчас это настолько широко распространено, что общепринято. Вы увидите это повсюду в телевизионных новостях или где угодно.
...
**Вы не можете использовать её для**, как я показывала в другом видео *(Урок 18)*, **формирования конструкции, которая означает «пытаться что-то сделать».**
...
И **это довольно тесно связано с другими субъективными прилагательными**, которые мы обсуждали в другом месте (и я дам ссылку на это), *(Урок 9)* **такими как <code>欲しい</code>, которое выражает наше желание чего-либо, или <code>怖い</code>, которое выражает наш страх перед чем-либо.**
...
Теперь, **это очень похоже на конструкцию**, о которой мы говорили раньше и которую вы, вероятно, уже знаете *(Урок 39)*, **<code>行くかどうか</code>**, что буквально означает **<code>пойду ли я или как</code>**, что в более естественном английском звучало бы как <code>whether I go or what</code>.

### Присоединение し к конструкции с まい

**Теперь, другая конструкция, которую вы часто услышите, образуется путём простого присоединения <code>し</code>, которое используется для перечисления причин чего-либо,** но, как мы обсуждали в другом месте *(Урок 63)*, **оно часто используется само по себе, чтобы просто подразумевать продолжение списка.**
