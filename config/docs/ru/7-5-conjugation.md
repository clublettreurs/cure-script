...
О каком именно спряжении?, спросите вы. Ну, обо всех. За исключением た- и て-форм, которые мы обсудим в другом видео *(Урок 81)*.
...
Итак, теперь мы знаем, как присоединять всё это, за исключением так называемых <code>る-глаголов</code>, или ичидан-глаголов, и они очень-очень просты, потому что всё, что вы когда-либо делаете с ними, это **просто отрезаете る, и у вас есть \*все\* возможные липкие основы ичидан-глаголов.**
...
Я уже упоминала об этом *(Урок 7)* — вероятно, наиболее распространённое использование — это присоединение вспомогательного прилагательного ない к липкой основе глагола.
...
А させる присоединяется к ичидан — так называемому <code>る-глаголу</code> — липкой основе.

Там, где есть небольшое изменение во вспомогательном глаголе, мы всегда обнаруживаем, что более длинная версия присоединяется к ичидан-глаголу, потому что этот глагол обычно короче.

---

У ичидан-глагола мы отрезаем весь последний слог; у годан-глаголов, меняющихся липких основ, мы не отрезаем последний слог, мы меняем его на другой звук.
...
::: info
はなす — это годан-глагол, заканчивающийся на す, поэтому он меняется на あ-основу + せる, что выглядит так же, как ичидан-каузативная форма させる. Долли объясняет подобные вещи в более поздних уроках.
:::
...
::: info
たべる — это ичидан-глагол, поэтому он просто удаляет る и добавляет させる, чтобы образовать каузатив.
:::
...
Итак, かう становится かわれる, что означает «быть купленным», «получить купленным» — **«получить купленным» лучше, потому что это ближе к тому, что на самом деле означает японский**, и вы поймёте это, когда посмотрите видео *(Урок 13)*;
...
**られる, как вы, возможно, заметили, совпадает с так называемым пассивом** *(рецептивом)*, れる / られる, так что ичидан-форма пассива и потенциала одинаковы** — но поскольку они используются очень по-разному, очень-очень мало случаев, когда вы могли бы их перепутать, так что это не проблема.
...
**В ичидан-форме мы добавляем よう к концу ичидан-липкой основы.** *(И, конечно, удаляем る)*
