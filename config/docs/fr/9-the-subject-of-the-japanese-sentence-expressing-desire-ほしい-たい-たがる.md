...
Et que signifie cet adjectif ? **Il ne signifie pas `vouloir` au sens français.** **Il ne le peut pas, car `vouloir` est un verbe et `たい`, se terminant par `い`, est un adjectif, n'est-ce pas ?** Prenons un exemple. C'est un exemple un peu notoire.
...
::: info
Notez le mot déclaration ici, たい ne peut pas être utilisé lorsque l'on `exprime une connaissance réelle de la subjectivité de quelqu'un d'autre` comme Dolly le dit dans les [**commentaires**](https://www.youtube.com/watch?v=vk3aKqMQwhM&lc=UgwE8cByEhTfyfe0e3h4AaABAg.9Kr0u3ynAsW9KrNJyZGL-o), il est recommandé de lire cela.
:::
